...
public class UserActionServiceImpl implements UserActionService {
    ...
    @Override
    public PriorityQueue<CategoryRankItem> getEventCategory() {
        KafkaStreams kafkaStreams = factoryBean.getKafkaStreams();
        var counts = kafkaStreams.store(
                StoreQueryParameters.fromNameAndType("categoryRank", QueryableStoreTypes.timestampedWindowStore())
        );
        PriorityQueue<CategoryRankItem> result = new PriorityQueue<>(Comparator.comparing(CategoryRankItem::getCount));
        counts.fetchAll(Instant.now().minus(Duration.ofSeconds(30)), Instant.now())
                .forEachRemaining(action -> {
                    result.add(new CategoryRankItem(String.valueOf(action.key.key()), (Long)(action.value.value())));
                });
